...
export default function Wallet() {
  ...
  async function handleSend() {
    ...
    setSendLoading(true); setSendStatus(null);
    try {
      ...
      const data = await res.json();
      if (data.success) { setSendStatus({ ok: true, msg: `✓ Sent ${sendAmount} sats to ${lnAddress}` }); setSendAmount(''); }
      else { setSendStatus({ ok: false, msg: data.error }); }
  ...
  return (
    <div style={{ maxWidth: 480 }}>
      ...
      <div style={{ display: 'flex', gap: 0, marginBottom: 28, background: '#111', borderRadius: 10, padding: 4 }}>
        {[['redeem', 'Redeem Gift Card'], ['send', 'Send sats']].map(([t, label]) => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {scanning ? (
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={startCamera} style={{ padding: '10px 16px', borderRadius: 10, background: '#1a1a1a', border: '1px solid #333', color: '#F7931A', fontFamily: 'var(--font-mono)', fontSize: 12, cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}>
                📷 QR Scan
              </button>
              <textarea
      ...
      {tab === 'send' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            ...
            <input type="text" placeholder="friend@walletofsatoshi.com" value={lnAddress} onChange={e => setLnAddress(e.target.value)}
              style={{ width: '100%', padding: '10px 14px', borderRadius: 10, background: '#111', border: '1px solid #333', color: '#fff', fontFamily: 'var(--font-mono)', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
          </div>
          <div>
            <span style={labelStyle}>AMOUNT (SATS)</span>
            <input type="number" placeholder="1000" value={sendAmount} onChange={e => setSendAmount(e.target.value)}
              style={{ width: '100%', padding: '10px 14px', borderRadius: 10, background: '#111', border: '1px solid #333', color: '#fff', fontFamily: 'var(--font-mono)', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
          </div>
          <button onClick={handleSend} disabled={!lnAddress || !sendAmount || sendLoading} style={{
            width: '100%', padding: '14px', borderRadius: 10,
            background: lnAddress && sendAmount ? '#F7931A' : '#1a1a1a',
            color: lnAddress && sendAmount ? '#000' : '#444',
            fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 15,
            border: 'none', cursor: lnAddress && sendAmount ? 'pointer' : 'not-allowed', transition: 'all 0.2s',
          }}>
            {sendLoading ? 'Sending...' : 'Send ⚡'}
          </button>
          {sendStatus && (
            <div style={{ padding: '14px', borderRadius: 10, background: sendStatus.ok ? '#0d1a0d' : '#1a0d0d', border: `1px solid ${sendStatus.ok ? '#1a3a1a' : '#3a1a1a'}`, fontFamily: 'var(--font-mono)', fontSize: 12, color: sendStatus.ok ? '#39ff14' : '#ff4444' }}>
              {sendStatus.msg}
            </div>
          )}
        </div>
